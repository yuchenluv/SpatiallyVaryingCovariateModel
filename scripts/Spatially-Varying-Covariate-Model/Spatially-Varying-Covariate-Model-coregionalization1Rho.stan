...
parameters{
  ...
  real<lower=0> rho; // kernel length scale
  real<lower=0> alpha; // kernel standard deviation
...
model{
  matrix[n_station, n_station] K_xi; // kernel for the latent GP
  ...
  mu_beta ~ multi_normal_cholesky(rep_vector(0, n_station), mu_w * K_xi);
  mu0 ~ multi_normal_cholesky(rep_vector(0, n_station), mu0_w * K_xi);
  logs_beta ~ multi_normal_cholesky(rep_vector(0, n_station), logs_w * K_xi);
  logs0 ~ multi_normal_cholesky(rep_vector(0, n_station), logs0_w * K_xi);
