...
```{julia}
function map_points_subplots(points_df, all_data, colorschemes, rows, cols, row_names, col_names, res, row_hs, title_names, ranges; diff_coord=false, bar_all=true, diff_colbar=false, bar_name=false)
    # bar_all: if would need color bar for all subplots
    # diff_colbar: if different color schemes for the color bars
    # point_df: stations with coordinates
    states = download(
        "https://raw.githubusercontent.com/PublicaMundi/MappingAPI/master/data/geojson/us-states.json",
    )
...
Functions for return level calculations

```{julia}
# nonstationary return level given GEV parameters
function rl_estimate1(p, x, μ0, μ_beta, logσ0, logσ_beta, ξ)
    μ = μ0 + x * μ_beta
    σ = exp(logσ0 + x * logσ_beta)
    return quantile(GeneralizedExtremeValue(μ, σ, ξ), p)
end

# estimate for stations with multiGP nonstationary
function rl_estimate4_multiGP(point_df, μ_beta, logσ_beta, μ0_posterior, logσ0_posterior, ξ_posterior, x, p)
...
# estimate for stations with nonpooled nonstationary
function rl_estimate4_nonpooled(point_df, μ_beta, logσ_beta, μ0_posterior, logσ0_posterior, ξ_posterior, x, p)
    mean_stations = zeros(size(point_df)[1])
    std_stations = zeros(size(point_df)[1])
    for k in 1:size(point_df)[1]
        μ_beta_k = μ_beta[:, k]
...
# estimate for stations with pooled stationary
function rl_etimate4_stationary(point_df, mu, logs, xi, x, p)
    mean_stations = zeros(size(point_df)[1])
    std_stations = zeros(size(point_df)[1])
    for k in 1:size(point_df)[1]
        mu_k = mu[:, k]
...
```{julia}
# station info along with length of records available
raw_data_df = DataFrame(CSV.File(datadir("processed/raw_1d/raw_data_map_30y.csv")))
raw_data_df_shuffled = DataFrame(CSV.File(datadir("processed/raw_1d/raw_data_map_shuffled_30y.csv")))
```

Get the covaraites (logCO2)

```{julia}
log_CO2 = raw_data_all[:, size(raw_data_all)[2]-1]
...
all_data = [raw_data_df[:, :n_obs], raw_data_df_shuffled[37:181, :][:, :n_obs], vcat(raw_data_df_shuffled[1:36, :], raw_data_df_shuffled[73:181, :])[:, :n_obs], vcat(raw_data_df_shuffled[1:72, :], raw_data_df_shuffled[109:181, :])[:, :n_obs], vcat(raw_data_df_shuffled[1:108, :], raw_data_df_shuffled[145:181, :])[:, :n_obs], raw_data_df_shuffled[1:144, :][:, :n_obs]]
```
